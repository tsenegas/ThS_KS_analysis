...
Dans le cadre de cette analyse technique en science des données, j'ai choisi d'utiliser et découvrir le nouvel IDE de Posit (anciennement Rstudio), [positron](https://github.com/posit-dev/positron). Ce nouvel IDE offre une intégration fluide entre différents langages de programmation, notamment R et Python mais également Julia ou SQL. En tant qu'utilisateur principal de R, j'ai pu réaliser une première étude exploratoire du jeu de données en R avant de créer un modèle ML en Python. Ce nouvel IDE, positron, m'a permis de jongler entre les deux langages au sein du même projet de manière très simple et fluide.
